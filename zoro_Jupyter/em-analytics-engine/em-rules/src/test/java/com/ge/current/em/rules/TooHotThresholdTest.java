 ...
 */
public class TooHotThresholdTest extends RuleBaseTest {

    private static final Logger LOG = LoggerFactory.getLogger(TooHotThresholdTest.class);
    ...
    private static class MockObjectFactory {
        ...
        public static List<Map<String, Object>> createFaultMeasuresMap() {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                int zoneAirTempSensor = rand.nextInt(5) + 76;
                Date timestamp = new Date(now + (15 * i * ONE_MINUTE_IN_MILLIS));
                currentMeasures.putAll(createMap(TagConstants.Measures.ZONE_AIR_TEMP_SENSOR.getMeasureName(),
                        new Double(zoneAirTempSensor)));
        ...
        public static List<Map<String, Object>> createNonFaultMeasureMap() {
            ...
            for (int i = 0; i < 4; i++) {
                Map<String, Object> currentMeasures = new LinkedHashMap<>();
                Random rand = new Random();
                int zoneAirTempSensor = rand.nextInt(5) + 69;
                Date timestamp = new Date(now + (15 * i * ONE_MINUTE_IN_MILLIS));
                currentMeasures.putAll(createMap(TagConstants.Measures.ZONE_AIR_TEMP_SENSOR.getMeasureName(),
                        new Double(zoneAirTempSensor)));
                currentMeasures.putAll(createMap(TagConstants.Measures.EVENT_TS.getMeasureName(), timestamp));
        ...
        public static Map<String, Object> createParameters() {
            ...
            parameters.putAll(createMap(TagConstants.Parameters.COMFORT_THRESHOLD.getParameterName() + RulesUtil
                    .getFDSINumber(RulesUtil.TOO_HOT_THRESHOLD), 75.0));
            parameters.putAll(createMap(TagConstants.Parameters.DURATION.getParameterName() + RulesUtil
                    .getFDSINumber(RulesUtil.TOO_HOT_THRESHOLD), 30.0));
            parameters.putAll(createMap(TagConstants.Parameters.SEVERITY.getParameterName() + RulesUtil
                    .getFDSINumber(RulesUtil.TOO_HOT_THRESHOLD), 1.0));
            return parameters;
        }
    }
}
