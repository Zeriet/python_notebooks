 ...
 */
public class ExcessiveLightingOverridesCountTest extends RuleBaseTest {

    private static Logger LOGGER = LoggerFactory.getLogger(ExcessiveLightingOverridesCountTest.class);
    ...
    public  void itShouldDetectNoFault(){
        ...
        RulesBaseFact rulesBaseFact = MockObjectFactory.getFacts();
        rulesBaseFact.setMeasuresAggrMap(MockObjectFactory.createMeasuresMapWithNoDetectFault());
        rulesBaseFact.setParameters(MockObjectFactory.createParameters());
        IERulesUtil.initRulesEngine(rules);
        List<RulesBaseFact> list = new ArrayList<>();
        list.add(rulesBaseFact);
    ...
    private static class MockObjectFactory {
        ...
        public static List<Map<String, Object>> createMeasuresMapWithDetectFault() {
            List<Map<String, Object>> meausuresAgg = new LinkedList<>();
            for (int i = 0; i < 12; i++) {
                Map<String, Object> measuresAggMap = new HashMap<>();
                measuresAggMap.putAll(createTags(TagConstants.Tags.LIGHTING_STATE_OVERRIDE.getTagName(), 12.0));
                measuresAggMap.putAll(createTags(TagConstants.Measures.EVENT_TS.getMeasureName(), "1234"));
                meausuresAgg.add(measuresAggMap);
            }
        ...
        public static List<Map<String, Object>> createMeasuresMapWithNoDetectFault() {
            ...
            for (int i = 0; i < 1; i++) {
                ...
                currenTagsMap.putAll(createTags("tagNotlightingStateOverride06", 6.0));
                measuresMap.add(currenTagsMap);
            }
        ...
        public static Map<String, Object> createParameters() {
            ...
            parameters.put(TagConstants.Parameters.USER_THRESHOLD.getParameterName()  + RulesUtil.getFDSINumber(RulesUtil.EXCESSIVE_LIGHTING_OVERRIDES_COUNT), 10.0d);
            return parameters;
        }
    }
}
