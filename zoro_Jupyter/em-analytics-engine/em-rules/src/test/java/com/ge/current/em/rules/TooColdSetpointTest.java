...
/**
 * Created by 212577826 on 1/4/17.
 * Test for FDSI-21: Zone temperature too low
 */
public class TooColdSetpointTest extends RuleBaseTest {

    private static final Logger LOG = LoggerFactory.getLogger(TooColdSetpointTest.class);
    ...
    public void testPositiveUseCase() {
        RulesBaseFact rulesBaseFact = TooColdSetpointTest.MockObjectFactory.getFacts();
        rulesBaseFact.setMeasuresAvgMap(TooColdSetpointTest.MockObjectFactory.createMeasuresMapWithDetectFault());
    ...
    @Test
    public void testNegativeUseCase() {
        RulesBaseFact rulesBaseFact = TooColdSetpointTest.MockObjectFactory.getFacts();
        rulesBaseFact
                .setMeasuresAvgMap(TooColdSetpointTest.MockObjectFactory.createMeasuresMapWithNoDetectFault());
    ...
    private static class MockObjectFactory {
        ...
        public static List<Map<String, Object>> createMeasuresMapWithDetectFault() {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                currentMeasures.putAll(createMap(TagConstants.Measures.ZONE_AIR_TEMP_SENSOR.getMeasureName(), 70.0));
                currentMeasures
                        .putAll(createMap(TagConstants.Tags.ZONE_AIR_TEMP_EFFECTIVE_SP_HEATING.getTagName(), 73.0));
        ...
        public static List<Map<String, Object>> createMeasuresMapWithNoDetectFault() {
            ...
            for (int i = 0; i < 4; i++) {
                Map<String, Object> currentMeasures = new HashMap<>();
                Date timestamp = new Date(now + (15 * i * ONE_MINUTE_IN_MILLIS));
                currentMeasures.putAll(createMap(TagConstants.Measures.ZONE_AIR_TEMP_SENSOR.getMeasureName(), 68.0));
                currentMeasures
                        .putAll(createMap(TagConstants.Tags.ZONE_AIR_TEMP_EFFECTIVE_SP_HEATING.getTagName(), 70.0));
                currentMeasures.putAll(createMap(TagConstants.Measures.EVENT_TS.getMeasureName(), timestamp.getTime()));
                measuresMap.add(currentMeasures);
            }
            return measuresMap;
